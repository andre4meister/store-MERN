...
const alert = createSlice({
  ...
  reducers: {
    ...
    addNotification: (state, action: PayloadAction<NotificationsType>) => {
      const isTheSameMessage = state.notifications.find((n) => n.message === action.payload.message);
      if (isTheSameMessage) {
        state.notifications = [
          ...state.notifications.filter((n) => n.message !== action.payload.message),
          action.payload,
        ];
      } else {
    ...
    deleteNotification: (state, action: PayloadAction<NotificationsType>) => {
      state.notifications = state.notifications.filter((n) => n.message !== action.payload.message);
    },
  },
});
