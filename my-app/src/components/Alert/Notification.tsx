import { CheckOutlined, CloseOutlined, InfoOutlined, WarningFilled } from '@ant-design/icons';
import { ActionCreatorWithPayload } from '@reduxjs/toolkit';
import cn from 'classnames';
import { useAppDispatch } from '../../hooks/redux';
import { FC, useEffect, useRef, useState } from 'react';
import { CSSTransition } from 'react-transition-group';
import { NotificationsType } from '../../store/alert/alert';
...
const Notification: FC<NotificationProps> = ({ notification, deleteNotification }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {notification.type === 'success' && <CheckOutlined className={styles.icon} />}
          {notification.type === 'info' && <InfoOutlined className={styles.icon} />}
          {notification.type === 'error' && <WarningFilled className={styles.icon} />}
        </div>
        <span className={styles.messageText}>{notification.message}</span>
        <CloseOutlined className={cn(styles.icon, styles.closeIcon)} title="Close this" onClick={onClickClose} />
      </div>
    </CSSTransition>
  );
};
